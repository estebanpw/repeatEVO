...
int main(int ac, char** av) {
	...
	fprintf(stdout, "[WARNING] - THIS SCRIPT IS TO BE USED ONLY WITHIN THE repeatEVO SUITE - DO NOT USE THIS SCRIPT AS STANDALONE\n");
	...
	while (!feof(fFrags) && exit == 0) {
		...
		if(frag.strand == 'f'){
			...
			int64_t border_left_size = frag.yStart - lefty;
			int64_t border_right_size = righty - frag.yEnd;
